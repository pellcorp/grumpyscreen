  ...
  , back_btn(panel_cont, Icons::BACK, "Back", &FineTunePanel::_handle_back, this)
{
  ...
  // the selectors and Back share one fixed-height row, so the four parameter
  // rows above get everything else
  const int bottom_h = scale_r(64);
  for (Selector *s : {&multiplier_selector, &step_selector}) {
    lv_obj_set_height(s->get_container(), bottom_h);
  }
  lv_obj_set_grid_cell(multiplier_selector.get_container(), LV_GRID_ALIGN_STRETCH, 0, 1, LV_GRID_ALIGN_CENTER, 4, 1);
  lv_obj_set_grid_cell(step_selector.get_container(), LV_GRID_ALIGN_STRETCH, 1, 2, LV_GRID_ALIGN_CENTER, 4, 1);
  back_btn.use_card();
  lv_obj_set_height(back_btn.get_container(), bottom_h);
  lv_obj_set_grid_cell(back_btn.get_container(), LV_GRID_ALIGN_STRETCH, 3, 1, LV_GRID_ALIGN_CENTER, 4, 1);
