  ...
  , delete_btn(bottom_cont, Icons::DELETE_IMG, "", &ConsolePanel::_handle_delete_btn, this)
{
  ...
  // the line has focus and sends on enter
  // the entry line and its keyboard share a box that is content-sized while
  // the keyboard is away and half the tab while it is up; the log takes the rest
  ...
  lv_obj_set_size(delete_btn.get_container(), scale_r(34), scale_r(34));  // the entry line's height
...
void ConsolePanel::foreground() {
  ws.send_jsonrpc("server.gcode_store", R"({"count": 100})"_json, [this](json &d) {
    ...
    for (auto &e : store) {
      ...
      ta_add_text_limit_lines(output, (e["type"] == "command" ? "> " : "") + strip_tags(msg));
    }
  });
}
