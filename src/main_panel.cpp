...
MainPanel::MainPanel(KWebSocketClient &websocket,
  ...
  , fan_panel(ws, lock)
  , led_panel(ws, lock)    
  , tabview(lv_tabview_create(lv_scr_act(), LV_DIR_LEFT, scale_w(60)))
...
void MainPanel::create_panel() {
  ...
  lv_obj_set_style_text_color(tab_btns, col(TEXT_DIM), LV_PART_ITEMS);
  // the selected tab: raised grey slot, accent-coloured glyph
  ...
  lv_obj_set_style_bg_color(tab_btns, col(RAISED), LV_STATE_CHECKED | LV_PART_ITEMS);
