...
static inline lv_obj_t * create_configurable_dialog(lv_obj_t * parent,
    ...
    lv_obj_set_style_pad_hor(title_label, Theme::gap() * 3 / 2, 0);
    ...
    lv_obj_set_style_pad_all(content, Theme::gap(), 0);
    // the text area grows with the message but never past what the screen
    // can show with the title and buttons still visible: beyond that it
    // scrolls. Box padding is gap()*1.5 top and bottom, the two row gaps are
    // one gap() each, the title is its line plus a gap() each side.
    const lv_coord_t button_h = Theme::touch_h();
    lv_obj_set_style_max_height(content,
        Theme::popout_max_h() - 2 * Theme::popout_pad() - 2 * Theme::gap()
        - (lv_font_get_line_height(Theme::scale_font(18)) + 2 * Theme::gap())
        - button_h, 0);

    // The box hugs its message: short text makes a small box with even air all
    // round it, long text wraps at the box's maximum width. Measured up front,
    // because percent-sized children (title bar, button row) cannot size a
    // content-sized parent.
    const lv_coord_t box_max_w = std::min(Theme::popout_w(), Theme::scale_w(400));
    const lv_coord_t side_pad = 2 * Theme::popout_pad() + 2 * Theme::gap();
    lv_point_t text_size, title_size;
    lv_txt_get_size(&text_size, message, Theme::scale_font(16), 0, 0, box_max_w - side_pad, LV_TEXT_FLAG_NONE);
    lv_txt_get_size(&title_size, title, Theme::scale_font(18), 0, 0, box_max_w - side_pad, LV_TEXT_FLAG_NONE);
    const lv_coord_t box_w = std::clamp<lv_coord_t>(std::max(text_size.x, title_size.x) + side_pad,
