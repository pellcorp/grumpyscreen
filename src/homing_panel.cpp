		     ...
		     {".1", ".5", "1", "5", "10", "25", "50", ""}, 2, &HomingPanel::_handle_selector_cb, this)
{
  ...
  // the selector and Back share one fixed-height row (Back shrinks its icon to
  // fit, like every tile); the two tile rows above take the rest
  const int bottom_h = Theme::scale_r(64);
  lv_obj_set_height(distance_selector.get_container(), bottom_h);
  lv_obj_set_grid_cell(distance_selector.get_container(), LV_GRID_ALIGN_STRETCH, 0, 4, LV_GRID_ALIGN_CENTER, 2, 1);
  back_btn.use_card();
  lv_obj_set_height(back_btn.get_container(), bottom_h);
  lv_obj_set_grid_cell(back_btn.get_container(), LV_GRID_ALIGN_STRETCH, 4, 1, LV_GRID_ALIGN_CENTER, 2, 1);
