#include "slider_container.h"
#include "logger.h"
#include "theme.h"

using namespace Theme;

SliderContainer::SliderContainer(lv_obj_t *parent,
				 const char *label_text,
				 const void *off_btn_img,
				 const char *off_text,
				 const void *max_btn_img,
				 const char *max_text,
				 lv_event_cb_t cb,
				 void *user_data,
				 std::string u)
  : SliderContainer(parent,
		    label_text,
		    off_btn_img,
		    off_text,
		    cb,
		    user_data,
		    max_btn_img,
		    max_text,
		    cb,
		    user_data,
		    cb,
		    user_data,
		    u) {
}

SliderContainer::SliderContainer(lv_obj_t *parent,
				 const char *label_text,
				 const void *off_btn_img,
				 const char *off_text,
				 lv_event_cb_t off_cb,
				 void * off_cb_user_data,
				 const void *max_btn_img,
				 const char *max_text,
				 lv_event_cb_t max_cb,
				 void * max_cb_user_data,
				 lv_event_cb_t slider_cb,
				 void * slider_user_data,
				 std::string u)
  : cont(lv_obj_create(parent))
  , label(lv_label_create(cont))
  , control_cont(create_row(cont))
  , off_btn(control_cont, off_btn_img, off_text, off_cb, off_cb_user_data)
  , slider_cont(create_row(control_cont))
  , slider(lv_slider_create(slider_cont))
  , slider_value(lv_label_create(slider_cont))
  , max_btn(control_cont, max_btn_img, max_text, max_cb, max_cb_user_data)
  , unit(u)
{
  lv_obj_clear_flag(cont, LV_OBJ_FLAG_SCROLLABLE);
  lv_obj_set_flex_flow(cont, LV_FLEX_FLOW_ROW);
  lv_obj_set_flex_align(cont, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
  lv_obj_add_style(cont, &styles().row_card, 0);
  lv_obj_set_size(cont, lv_pct(100), LV_SIZE_CONTENT);

  // the name sits centred above the slider
  lv_obj_set_parent(label, slider_cont);
  lv_obj_move_to_index(label, 0);
  lv_label_set_text(label, label_text);
  lv_obj_set_width(label, LV_PCT(100));
  lv_obj_set_style_text_align(label, LV_TEXT_ALIGN_CENTER, 0);
  lv_label_set_long_mode(label, LV_LABEL_LONG_DOT);

  // Off and Max are plain icon-over-label tiles (no card: the slider row is
  // the card) as tall as the row the panel gives it; the icon fits itself in
  for (ButtonContainer *b : {&off_btn, &max_btn}) {
    b->use_plain();
    lv_obj_set_size(b->get_container(), scale_w(64), scale_r(60));  // until set_height() says otherwise
  }

  lv_obj_set_size(control_cont, 0, LV_SIZE_CONTENT);
  lv_obj_set_flex_grow(control_cont, 1);
  lv_obj_set_flex_flow(control_cont, LV_FLEX_FLOW_ROW);
  lv_obj_set_flex_align(control_cont, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);

  lv_obj_set_size(slider_cont, 0, LV_SIZE_CONTENT);
  lv_obj_set_flex_grow(slider_cont, 1);
  // name / slider / value, stacked and centred
  lv_obj_set_flex_flow(slider_cont, LV_FLEX_FLOW_COLUMN);
  lv_obj_set_flex_align(slider_cont, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
  lv_obj_set_width(slider, LV_PCT(100));

  if (max_text == NULL) {
    max_btn.hide();
  }

  lv_label_set_text(slider_value, "");
  if (slider_cb != NULL) {
    lv_obj_add_event_cb(slider, slider_cb, LV_EVENT_RELEASED, slider_user_data);
  } else {
    lv_obj_add_flag(slider, LV_OBJ_FLAG_HIDDEN);
  }
  lv_obj_add_event_cb(slider, &SliderContainer::_handle_value_update, LV_EVENT_VALUE_CHANGED, this);
}

SliderContainer::~SliderContainer() {
  if (cont != NULL) {
    lv_obj_del(cont);
    cont = NULL;
  }
}

lv_obj_t *SliderContainer::get_container() {
  return cont;
}

lv_coord_t SliderContainer::row_height(int n) {
  const int col_h = lv_disp_get_physical_ver_res(NULL) - 2 * gap();  // a screen root's content height
  return (n == 2 || n == 3) ? (col_h - (n - 1) * gap()) / n : (col_h - 2 * gap()) / 3;
}

void SliderContainer::set_height(lv_coord_t h) {
  lv_obj_set_height(cont, h);
  // explicit, not a percentage: a percent child of a row whose height was set
  // after it was built stays at its first, empty, size
  const lv_coord_t inner = h - 2 * gap() - 2 * lv_obj_get_style_border_width(cont, 0);
  for (ButtonContainer *b : {&off_btn, &max_btn}) lv_obj_set_height(b->get_container(), inner);
}

lv_obj_t *SliderContainer::get_slider() {
  return slider;
}

lv_obj_t *SliderContainer::get_off() {
  return off_btn.get_container();
}

lv_obj_t *SliderContainer::get_max() {
  return max_btn.get_container();
}

void SliderContainer::set_range(int min_range, int max_range) {
  lv_slider_set_range(slider, min_range, max_range);
}

void SliderContainer::update_value(int value) {
  // no slider means a plain switch: a percentage would be a lie
  if (lv_obj_has_flag(slider, LV_OBJ_FLAG_HIDDEN)) {
    lv_label_set_text(slider_value, value > 0 ? "On" : "Off");
  } else {
    lv_label_set_text(slider_value, fmt::format("{}{}", value, unit).c_str());
  }
  lv_slider_set_value(slider, value, LV_ANIM_ON);
}

void SliderContainer::handle_value_update(lv_event_t *event) {
  lv_obj_t * obj = lv_event_get_target(event);
  update_value((int)lv_slider_get_value(obj));
}
